...
variable "iso_url" {
  type    = string
  default = "https://cdimage.debian.org/cdimage/archive/13.0.0/amd64/iso-cd/debian-13.0.0-amd64-netinst.iso"
}
...
variable "vm_name" {
  type    = string
  default = "debian-13-0-x64-fr-server-template"
}
...
locals {
  template_description = "Debian 13.0 template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => debian:debian"
}

source "proxmox-iso" "debian13" {
  boot_command = [
    "<down><tab>", # non-graphical install
    "<wait>preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-13-preseed.cfg ",
    "<wait>language=fr locale=fr_FR.UTF-8 ",
    "<wait>country=FR keymap=fr ",
    "<wait>hostname=debian13fr domain=local ",
    "<enter><wait>",
  ]

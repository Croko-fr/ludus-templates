...
variable "iso_url" {
  type    = string
  default = "https://cdimage.debian.org/mirror/cdimage/archive/12.0.0/amd64/iso-cd/debian-12.0.0-amd64-netinst.iso"
}
...
variable "vm_name" {
  type    = string
  default = "debian-12-0-x64-fr-server-template"
}
...
locals {
  template_description = "Debian 12.0 template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => debian:debian"
}
...
source "proxmox-iso" "debian12" {
  boot_command = [
    ...
    "<wait>hostname=debian12fr domain=local ",
    "<enter><wait>",
  ]
